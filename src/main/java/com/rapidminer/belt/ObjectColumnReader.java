 ...
 */
public final class ObjectColumnReader<T> {
	 ...
	 */
	public ObjectColumnReader(Column src, Class<T> type) {
		this(src, type, ColumnReader.DEFAULT_BUFFER_SIZE, src.size());
	}
	 ...
	 * 		if type is not a super type of the column element type
	 */
	public ObjectColumnReader(Column src, Class<T> type, int length) {
		this(src, type, ColumnReader.DEFAULT_BUFFER_SIZE, length);
	}
	 ...
	 * 		if type is not a super type of the column element type
	 */
	ObjectColumnReader(Column src, Class<T> type, int bufferSizeHint, int length) {
	...
	/**
	 * Returns current position in the column (0-based). Returns {@link Row#BEFORE_FIRST} if the reader is before the
	 * first position, i.e., {@link #read()} has not been called.
	 *
