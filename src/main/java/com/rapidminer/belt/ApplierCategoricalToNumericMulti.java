...
/**
 * Maps {@link Column.Category#CATEGORICAL} {@link Column}s to a {@link ColumnBuffer} using a given mapping operator.
 *
 * @author Gisa Meier
 */
final class ApplierCategoricalToNumericMulti implements ParallelExecutor.Calculator<ColumnBuffer> {
	...
	ApplierCategoricalToNumericMulti(Column[] sources, ToDoubleFunction<CategoricalRow> operator, boolean round) {
	 ...
	 */
	private static void mapPart(final Column[] sources, final ToDoubleFunction<CategoricalRow> operator, final ColumnBuffer target,
								int from, int to) {
