...
/**
 * Maps {@link Column.Capability#NUMERIC_READABLE} {@link Column}s to a {@link TimeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierNumericToTimeMulti implements ParallelExecutor.Calculator<TimeColumnBuffer> {


	private TimeColumnBuffer target;
	private final Column[] sources;
	private final Function<Row, LocalTime> operator;

	ApplierNumericToTimeMulti(Column[] sources, Function<Row, LocalTime> operator) {
	 ...
	 */
	private static void mapPart(final Column[] sources, final Function<Row, LocalTime> operator,
