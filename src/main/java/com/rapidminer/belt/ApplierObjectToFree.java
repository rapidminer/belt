...
/**
 * Maps a {@link Column.Capability#OBJECT_READABLE} {@link Column} to a {@link FreeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierObjectToFree<R,T> implements ParallelExecutor.Calculator<FreeColumnBuffer<T>> {
	...
	ApplierObjectToFree(Column source, Class<R> sourceType, Function<R,T> operator) {
		this.source = source;
	 ...
	 */
	private static <R, T> void mapPart(Column source, Class<R> sourceType, Function<R, T> operator, FreeColumnBuffer<T> target, int from, int to) {
