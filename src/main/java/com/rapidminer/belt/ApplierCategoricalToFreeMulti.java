...
/**
 * Maps {@link Column.Category#CATEGORICAL} {@link Column}s to a {@link FreeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierCategoricalToFreeMulti<T> implements ParallelExecutor.Calculator<FreeColumnBuffer<T>> {
	...
	ApplierCategoricalToFreeMulti(Column[] sources, Function<CategoricalRow, T> operator) {
	 ...
	 */
	private static <T> void mapPart(final Column[] sources, final Function<CategoricalRow, T> operator,
									final FreeColumnBuffer<T> target, int from, int to) {
