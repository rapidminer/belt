 ...
 * @author Gisa Meier
 */
final class ObjectColumnsFilterer<T> implements ParallelExecutor.Calculator<int[]> {
	...
	ObjectColumnsFilterer(Column[] sources, Class<T> sourceType, Predicate<ObjectRow<T>> operator) {
	 ...
	 */
	private static <T> int filterPart(Column[] sources, Class<T> type, boolean[] target,
