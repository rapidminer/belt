 ...
 * while building it. Internally it uses a chunked array representation to represent the double array. Adding elements
 * takes constant time. Elements will be added at the end of the array. Elements cannot be modified or randomly
 * accessed. Creating the final double array after filling the elements into the builder takes time linear in the number
 * of elements.
 *
...
public class DoubleArrayBuilder {
	 ...
	 */
	public DoubleArrayBuilder(ArrayBuilderConfiguration config) {
		this.config = config;
		currentChunkIndex = 0;
		size = 0;
		currentChunk = new double[config.initialChunkSize];
		chunkPool = new double[ArrayBuilderConfiguration.INITIAL_CHUNK_POOL_SIZE][];
	...
	/**
	 * Returns the stored double data in a newly created double array. The method takes time linear in the number of
	 * elements stored.
	 */
	public double[] getData() {
		...
		for (int intraChunkIndex = 0; intraChunkIndex < currentIndexInChunk; intraChunkIndex++) {
			result[resultIndex++] = currentChunk[intraChunkIndex];
		}
		return result;
	}
	...
	private void moveToNextChunk() {
		...
		currentChunk = new double[newLength];
	...
	private void growChunkPool() {
		double[][] newChunkPool = new double[chunkPool.length * 2][];
