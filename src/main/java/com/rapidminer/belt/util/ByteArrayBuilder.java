 ...
 * building it. Internally it uses a chunked array representation to represent the byte array. Adding elements takes
 * constant time. Elements will be added at the end of the array. Elements cannot be modified or randomly accessed.
 * Creating the final byte array after filling the elements into the builder takes time linear in the number of
 * elements.
 *
...
public class ByteArrayBuilder {
	 ...
	 */
	public ByteArrayBuilder(ArrayBuilderConfiguration config) {
		this.config = config;
		currentChunkIndex = 0;
		size = 0;
		currentChunk = new byte[config.initialChunkSize];
		chunkPool = new byte[ArrayBuilderConfiguration.INITIAL_CHUNK_POOL_SIZE][];
	...
	/**
	 * Returns the stored byte data in a newly created byte array. The method takes time linear in the number of
	 * elements stored.
	 */
	public byte[] getData() {
		...
		for (int chunkIndex = 0; chunkIndex < currentChunkIndex; chunkIndex++) {
			byte[] chunk = chunkPool[chunkIndex];
	...
	private void moveToNextChunk() {
		...
		currentChunk = new byte[newLength];
	...
	private void growChunkPool() {
		byte[][] newChunkPool = new byte[chunkPool.length * 2][];
