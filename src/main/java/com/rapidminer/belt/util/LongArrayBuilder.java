 ...
 * building it. Internally it uses a chunked array representation to represent the long array. Adding elements takes
 * constant time. Elements will be added at the end of the array. Elements cannot be modified or randomly accessed.
 * Creating the final long array after filling the elements into the builder takes time linear in the number of
 * elements.
 *
...
public class LongArrayBuilder {
	 ...
	 */
	public LongArrayBuilder(ArrayBuilderConfiguration config) {
		this.config = config;
		currentChunkIndex = 0;
		size = 0;
		currentChunk = new long[config.initialChunkSize];
		chunkPool = new long[ArrayBuilderConfiguration.INITIAL_CHUNK_POOL_SIZE][];
	...
	/**
	 * Returns the stored long data in a newly created long array. The method takes time linear in the number of
	 * elements stored.
	 */
	public long[] getData() {
		...
		for (int chunkIndex = 0; chunkIndex < currentChunkIndex; chunkIndex++) {
			long[] chunk = chunkPool[chunkIndex];
	...
	private void moveToNextChunk() {
		...
		currentChunk = new long[newLength];
	...
	private void growChunkPool() {
		long[][] newChunkPool = new long[chunkPool.length * 2][];
