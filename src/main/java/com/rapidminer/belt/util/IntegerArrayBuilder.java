 ...
 * while building it. Internally it uses a chunked array representation to represent the integer array. Adding elements
 * takes constant time. Elements will be added at the end of the array. Elements cannot be modified or randomly
 * accessed. Creating the final integer array after filling the elements into the builder takes time linear in the
 * number of elements.
 *
...
public class IntegerArrayBuilder {
	 ...
	 */
	public IntegerArrayBuilder(ArrayBuilderConfiguration config) {
		this.config = config;
		currentChunkIndex = 0;
		size = 0;
		currentChunk = new int[config.initialChunkSize];
		chunkPool = new int[ArrayBuilderConfiguration.INITIAL_CHUNK_POOL_SIZE][];
	...
	/**
	 * Returns the stored integer data in a newly created integer array. The method takes time linear in the number of
	 * elements stored.
	 */
	public int[] getData() {
		...
		for (int chunkIndex = 0; chunkIndex < currentChunkIndex; chunkIndex++) {
			int[] chunk = chunkPool[chunkIndex];
	...
	private void moveToNextChunk() {
		...
		currentChunk = new int[newLength];
	...
	private void growChunkPool() {
		int[][] newChunkPool = new int[chunkPool.length * 2][];
