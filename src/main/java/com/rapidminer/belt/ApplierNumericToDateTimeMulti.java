...
/**
 * Maps {@link Column.Capability#NUMERIC_READABLE} {@link Column}s to a {@link HighPrecisionDateTimeBuffer} using a
 * given mapping operator.
 *
 * @author Gisa Meier
 */
final class ApplierNumericToDateTimeMulti implements ParallelExecutor.Calculator<HighPrecisionDateTimeBuffer> {

	private HighPrecisionDateTimeBuffer target;
	private final Column[] sources;
	private final Function<Row, Instant> operator;

	ApplierNumericToDateTimeMulti(Column[] sources, Function<Row, Instant> operator) {
	 ...
	 */
	private static void mapPart(final Column[] sources, final Function<Row, Instant> operator,
