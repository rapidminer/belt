...
/**
 * Maps arbitrary {@link Column}s to a {@link HighPrecisionDateTimeBuffer} using a given mapping operator.
 *
 * @author Gisa Meier
 */
final class ApplierGeneralToDateTimeMulti implements ParallelExecutor.Calculator<HighPrecisionDateTimeBuffer> {


	private HighPrecisionDateTimeBuffer target;
	private final Column[] sources;
	private final Function<GeneralRow, Instant> operator;

	ApplierGeneralToDateTimeMulti(Column[] sources, Function<GeneralRow, Instant> operator) {
	 ...
	 */
	private static void mapPart(final Column[] sources, final Function<GeneralRow, Instant> operator,
