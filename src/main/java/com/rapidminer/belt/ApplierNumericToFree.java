...
/**
 * Maps a {@link Column.Capability#NUMERIC_READABLE} {@link Column} to a {@link FreeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierNumericToFree<T> implements ParallelExecutor.Calculator<FreeColumnBuffer<T>> {
	...
	ApplierNumericToFree(Column source, DoubleFunction<T> operator) {
		this.source = source;
	 ...
	 */
	private static <T> void mapPart(Column source, DoubleFunction<T> operator, FreeColumnBuffer<T> target, int from, int to) {
