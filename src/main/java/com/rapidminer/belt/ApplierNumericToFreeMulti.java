...
/**
 * Maps {@link Column.Capability#NUMERIC_READABLE} {@link Column}s to a {@link FreeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierNumericToFreeMulti<T> implements ParallelExecutor.Calculator<FreeColumnBuffer<T>> {
	...
	ApplierNumericToFreeMulti(Column[] sources, Function<Row, T> operator) {
	 ...
	 */
	private static <T> void mapPart(final Column[] sources, final Function<Row, T> operator, final FreeColumnBuffer<T> target,
									int from, int to) {
