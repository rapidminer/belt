...
/**
 * Maps {@link Column.Capability#OBJECT_READABLE} {@link Column}s to a {@link HighPrecisionDateTimeBuffer} using a given
 * mapping operator.
 *
 * @author Gisa Meier
 */
final class ApplierObjectToDateTimeMulti<R> implements ParallelExecutor.Calculator<HighPrecisionDateTimeBuffer> {
	...
	ApplierObjectToDateTimeMulti(Column[] sources, Class<R> type, Function<ObjectRow<R>, Instant> operator) {
	...
	private static <R> void mapPart(final Column[] sources, final Function<ObjectRow<R>, Instant> operator,
									final Class<R> type, final HighPrecisionDateTimeBuffer target, int from, int to) {
