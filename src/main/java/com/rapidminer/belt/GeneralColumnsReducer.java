 ...
 * @author Gisa Meier
 */
final class GeneralColumnsReducer<T> implements ParallelExecutor.Calculator<T> {


	private final Column[] sources;
	private final Supplier<T> supplier;
	private final BiConsumer<T, GeneralRow> reducer;
	private final BiConsumer<T, T> combiner;
	private ColumnReducer.CombineTree<T> combineTree;

	GeneralColumnsReducer(Column[] sources, Supplier<T> supplier, BiConsumer<T, GeneralRow> reducer, BiConsumer<T, T> combiner) {
	 ...
	 */
	private static <T> void reducePart(Column[] sources, T container, BiConsumer<T, GeneralRow> reducer, int from, int to) {
