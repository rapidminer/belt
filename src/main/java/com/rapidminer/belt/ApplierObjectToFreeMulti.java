...
/**
 * Maps {@link Column.Capability#OBJECT_READABLE} {@link Column}s to a {@link FreeColumnBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierObjectToFreeMulti<R, T> implements ParallelExecutor.Calculator<FreeColumnBuffer<T>> {
	...
	ApplierObjectToFreeMulti(Column[] sources, Class<R> type, Function<ObjectRow<R>, T> operator) {
	 ...
	 */
	private static <R, T> void mapPart(final Column[] sources, final Function<ObjectRow<R>, T> operator,
									   final Class<R> type, final FreeColumnBuffer<T> target, int from, int to) {
