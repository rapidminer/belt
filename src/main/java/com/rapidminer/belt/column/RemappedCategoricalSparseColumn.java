...
/**
 * Sparse implementation of a {@link CategoricalColumn} with a remapping that adjusts the data to the dictionary. It
 * uses a more memory efficient representation of the data than the default dense {@link SimpleCategoricalColumn} while
 * still being comparably fast or faster on sparse data. Caution: This implementation does not support the 2 bit and 4
 * bit format.
 *
...
class RemappedCategoricalSparseColumn extends CategoricalColumn {
	...
	/**
	 * This is used to remap the internal indices to adjust them to the {@link #dictionary}.
	 */
	private final int[] remapping;

	private static final String NOT_SUPPORTED_2BIT = "Remapped sparse categorical column does not support 2 bit data.";
	private static final String NOT_SUPPORTED_4BIT = "Remapped sparse categorical column does not support 4 bit data.";
	private static final String ILLEGAL_STEP_SIZE = "step size must not be smaller than 1";
	 ...
	 * PackedIntegers}. The dictionary maps the internal integer values to their corresponding categories. The remapping
	 * will be used to adjust the internal integer representation to the given dictionary. Caution: This implementation
	 * does not support 2 bit or 4 bit format data.
	 *
	 * @param type
	 * 		the column type.
	 * @param bytes
	 ...
	 * @param remapping
	 * 		will be used to adjust the internal integer representation to the given dictionary.
	 * @param defaultValue
	...
	/**
	 * Creates a new RemappedCategoricalSparseColumn with the given type, defaultValue and data. The dictionary maps the
	 * internal integer values to their corresponding categories. The remapping will be used to adjust the internal integer
	 * representation to the given dictionary.
	 *
	 * @param type
	 * 		the column type.
	 * @param data
	...
	void fillFromUInt8(double[] array, int rowIndex, int arrayOffset, int arrayStepSize) {
		if (arrayStepSize < 1) {
			throw new IllegalArgumentException(ILLEGAL_STEP_SIZE);
		}
		if (rowIndex < 0) {
			throw new ArrayIndexOutOfBoundsException(rowIndex);
		}
	...
	void fillFromUInt8(int[] array, int rowIndex) {
		...
		int remappedDefaultValue = remapping[defaultValueAsUnsignedInt];
		Arrays.fill(array, remappedDefaultValue);
		if (nonDefaultIndices.length == 0) {
			// only default values
			return;
		}
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			int nonDefaultPosition = nonDefaultIndices[nonDefaultIndex];
			while (nonDefaultPosition < maxRowIndex) {
				array[nonDefaultPosition - rowIndex] = remapping[Byte.toUnsignedInt(byteNonDefaultValues[nonDefaultIndex])];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	void fillFromUInt16(int[] array, int rowIndex) {
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			int nonDefaultPosition = nonDefaultIndices[nonDefaultIndex];
			while (nonDefaultPosition < maxRowIndex) {
				array[nonDefaultPosition - rowIndex] = remapping[Short.toUnsignedInt(shortNonDefaultValues[nonDefaultIndex])];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	void fillFromInt32(int[] array, int rowIndex) {
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			int nonDefaultPosition = nonDefaultIndices[nonDefaultIndex];
			while (nonDefaultPosition < maxRowIndex) {
				array[nonDefaultPosition - rowIndex] = remapping[intNonDefaultValues[nonDefaultIndex]];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	void fillFromUInt8(int[] array, int rowIndex, int arrayOffset, int arrayStepSize) {
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			...
			while (nonDefaultPosition < maxRowIndex) {
				array[(nonDefaultPosition - rowIndex) * arrayStepSize + arrayOffset] =
						remapping[Byte.toUnsignedInt(byteNonDefaultValues[nonDefaultIndex])];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	void fillFromUInt16(int[] array, int rowIndex, int arrayOffset, int arrayStepSize) {
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			...
			while (nonDefaultPosition < maxRowIndex) {
				array[(nonDefaultPosition - rowIndex) * arrayStepSize + arrayOffset] =
						remapping[Short.toUnsignedInt(shortNonDefaultValues[nonDefaultIndex])];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	void fillFromInt32(int[] array, int rowIndex, int arrayOffset, int arrayStepSize) {
		...
		if (nonDefaultIndex < nonDefaultIndices.length) {
			...
			while (nonDefaultPosition < maxRowIndex) {
				array[(nonDefaultPosition - rowIndex) * arrayStepSize + arrayOffset] =
						remapping[intNonDefaultValues[nonDefaultIndex]];
				nonDefaultIndex++;
				if (nonDefaultIndex >= nonDefaultIndices.length) {
					break;
				}
	...
	private Column mapInt32(int[] mapping) {
		...
		if (mapping.length * CategoricalSparseColumn.MAX_DENSITY_INT32 < numberOfNonDefaults) {
	...
	private Column mapUInt16(int[] mapping) {
		...
		int numberOfNonDefaults = bitMap.countNonDefaultIndices(mapping);
		if (mapping.length * CategoricalSparseColumn.MAX_DENSITY_UINT16 < numberOfNonDefaults) {
			// column is not sparse enough anymore
			return makeDenseColumn(mapping);
		}
	...
	private Column mapUInt8(int[] mapping) {
		...
		int numberOfNonDefaults = bitMap.countNonDefaultIndices(mapping);
		if (mapping.length * CategoricalSparseColumn.MAX_DENSITY_UINT8 < numberOfNonDefaults) {
			// column is not sparse enough anymore
			return makeDenseColumn(mapping);
		}
	...
	private int[] getSortedMappingUINT8(Order order, Comparator<String> comparatorWithNull) {
		...
		return Sorting.sort(toSort.length,
				(a, b) -> comparatorWithNull.compare(toObject(Byte.toUnsignedInt(toSort[a])),
						toObject(Byte.toUnsignedInt(toSort[b]))), order);
	}
	...
	private int[] getSortedMappingUINT16(Order order, Comparator<String> comparatorWithNull) {
		...
		return Sorting.sort(toSort.length,
				(a, b) -> comparatorWithNull.compare(toObject(Short.toUnsignedInt(toSort[a])),
						toObject(Short.toUnsignedInt(toSort[b]))), order);
	}
	...
	private int[] getSortedMappingINT32(Order order, Comparator<String> comparatorWithNull) {
		...
		return Sorting.sort(toSort.length,
				(a, b) -> comparatorWithNull.compare(toObject(toSort[a]),
						toObject(toSort[b])), order);
	}

	/**
	 * Return the corresponding double value or {@link Double#NaN} iff the given value is equal to {@link
	 * CategoricalReader#MISSING_CATEGORY}. Uses remapping to remap the index before converting it to double.
	 */
	private double toDouble(int i) {
		int remapped = remapping[i];
		if (remapped == CategoricalReader.MISSING_CATEGORY) {
			return Double.NaN;
		} else {
			return remapped;
		}
