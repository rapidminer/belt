 ...
 * @author Gisa Meier
 */
final class ApplierNCategoricalToCategorical implements Calculator<NominalBuffer> {


	private NominalBuffer target;
	private final List<Column> sources;
	private final Function<CategoricalRow, String> operator;
	private final IntegerFormats.Format format;
	private final ColumnType<String> targetType;

	ApplierNCategoricalToCategorical(List<Column> sources, Function<CategoricalRow, String> operator, IntegerFormats.Format format, ColumnType<String> targetType) {
	 ...
	 */
	private static void mapPart(List<Column> sources, Function<CategoricalRow, String> operator, Int32NominalBuffer target,
									int from, int to) {
	 ...
	 */
	private static void mapPart(List<Column> sources, Function<CategoricalRow, String> operator, UInt16NominalBuffer target,
									int from, int to) {
		final CategoricalRowReader reader = Readers.categoricalRowReader(sources);
		reader.setPosition(from - 1);
		for (int i = from; i < to; i++) {
	 ...
	 */
	private static void mapPart(List<Column> sources, Function<CategoricalRow, String> operator, UInt8NominalBuffer target,
									int from, int to) {
		final CategoricalRowReader reader = Readers.categoricalRowReader(sources);
		reader.setPosition(from - 1);
		for (int i = from; i < to; i++) {
