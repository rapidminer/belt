...
/**
 * Tests {@link ColumnFilterer} and {@link ColumnsFilterer}.
 *
 * @author Gisa Meier
 */
@RunWith(Enclosed.class)
public class ColumnFiltererTests {
	...
	public static class Numeric {
		...
		public void testOneColumnPart() {
			...
			ColumnFilterer calculator = new ColumnFilterer(new DoubleArrayColumn(data), i -> i > 0.5);
			calculator.init(1);
			int start = 10;
			int end = 30;
		...
		public void testOneColumnWhole() {
			int size = 75;
			double[] data = random(size);
			Table table = new Table(new Column[]{new DoubleArrayColumn(data)}, new String[]{"a"});
		...
		public void testMoreColumnsPart() {
			...
			ColumnsFilterer calculator = new ColumnsFilterer(new Column[]{new DoubleArrayColumn(first),
					new DoubleArrayColumn(second), new DoubleArrayColumn(third)}, row -> row.get(0) + row.get(1) + row.get(2) > 1);
			calculator.init(1);
			int start = 10;
			int end = 30;
			calculator.doPart(start, end, 0);
		...
		public void testMoreColumnsWhole() {
			...
			Table table = new Table(new Column[]{new DoubleArrayColumn(first),
					new DoubleArrayColumn(second), new DoubleArrayColumn(third)}, new String[]{"a", "b", "c"});
	...
	public static class Categorical {
		...
		public void testOneColumnPart() {
			...
			CategoricalColumnFilterer calculator = new CategoricalColumnFilterer(
					new SimpleCategoricalColumn<>(ColumnTypes.NOMINAL, data, new ArrayList<>()), i -> i > 5);
			calculator.init(1);
			int start = 10;
			int end = 30;
			calculator.doPart(start, end, 0);
		...
		public void testOneColumnWhole() {
			int size = 75;
			int[] data = randomInt(size);
			Table table = new Table(new Column[]{
					new SimpleCategoricalColumn<>(ColumnTypes.NOMINAL, data, new ArrayList<>())}, new String[]{"a"});
		...
		public void testMoreColumnsPart() {
			...
			CategoricalColumnsFilterer calculator = new CategoricalColumnsFilterer(new Column[]{
	...
	public static class Objects {
		...
		public void testOneColumnWhole() {
			...
			List<String> mappingList = getMappingList();
			Table table = new Table(new Column[]{
					new SimpleCategoricalColumn<>(ColumnTypes.NOMINAL, data, mappingList)}, new String[]{"a"});
		...
		public void testMoreColumnsPart() {
			...
			int[] third = randomInt(size);
			List<String> mappingList = getMappingList();
			ObjectColumnsFilterer<String> calculator = new ObjectColumnsFilterer<>(new Column[]{
