...
public class ObjectRowReaderTests {
	...
	public static class Reading {
		...
		public void testReadingFromTwoColumns() {
			...
			ObjectRowReader<String> reader = new ObjectRowReader<>(String.class, columns[2], columns[0]);
			String[][] outputs = readAllColumnsToArrays(reader);
		...
		public void testReadingWholeTable() {
			...
			ObjectRowReader<String> reader = new ObjectRowReader<>(table, String.class);
			String[][] outputs = readAllColumnsToArrays(reader);
		...
		public void testReadingFromThreeColumns() {
			...
			ObjectRowReader<String> reader = new ObjectRowReader<>(String.class, table.column("c"), table.column("b"), table.column("a"));
			String[][] outputs = readAllColumnsToArrays(reader);
	...
	public static class Remaining {
		...
		public void testWrongClass() {
			new ObjectRowReader<>(new Column[]{column(0, new String[5])}, Boolean.class);
		}
		...
		public void testRightSuper() {
			...
			FreeColumn<Double> doubleCol = new FreeColumn<Double>(ColumnTypes.freeType(
				...
				void fill(Object[] buffer, int startIndex, int bufferOffset, int bufferStepSize) {
					...
					int max = Math.min(startIndex + buffer.length / bufferStepSize + remainder, 10);
			...
			ObjectRowReader<Number> reader = new ObjectRowReader<>(new Column[]{intCol, doubleCol}, Number.class);
			reader.move();
	...
	public static class Position {
		...
		public void testSetAfterReadsSmallerInSameBuffer() {
			...
			Column[] columns = new Column[nColumns];
			Arrays.setAll(columns, i -> column(i, testArray));
			ObjectRowReader<String> reader = new ObjectRowReader<>(columns, String.class, 10);
			for (int i = 0; i < 13; i++) {
				reader.move();
			}
			reader.setPosition(position);
