...
/**
 * Tests all implementations of {@link ParallelExecutor.Calculator}.
 *
 * @author Gisa Meier
 */
...
public class ColumnCalculatorTests {
	...
	public static Iterable<CalculatorTest<?>> tests() {
		...
		tests.add(new CalculatorTest<>("reduce_one", (data, data2) ->
				new ColumnReducer<>(new DoubleArrayColumn(data), () -> new double[2],
						(t, d) -> {
							t[0] += d;
							t[1] += 1;
						},
				...
				(data, data2) -> Arrays.stream(data).collect(() -> new double[2],
						...
						(t1, t2) -> {
							t1[0] += t2[0];
							t1[1] += t2[1];
						}), ParallelExecutor.Calculator::getResult));

		tests.add(new CalculatorTest<>("reduce_one_not_comm", (data, data2) ->
				new ColumnReducer<>(new DoubleArrayColumn(data), () -> new double[1],
						(t, d) -> {
							t[0] = d;
						},
						(t1, t2) -> {
				...
				(data, data2) -> new double[]{data[data.length - 1]}, ParallelExecutor.Calculator::getResult));

		tests.add(new CalculatorTest<>("map_one",
				(data, data2) -> new ApplierNumericToNumeric(new DoubleArrayColumn(data), i -> 2 * i, false),
				(data, data2) -> {
		...
		tests.add(new CalculatorTest<>("reduce_double", (data, data2) ->
				new ColumnReducerDouble(new DoubleArrayColumn(data), 0, Double::sum),
				(data, data2) -> {
		...
		tests.add(new CalculatorTest<>("reduce_double_combiner", (data, data2) ->
				new ColumnReducerDouble(new DoubleArrayColumn(data), 0,
		...
		tests.add(new CalculatorTest<>("reduce_double_not_comm", (data, data2) ->
				new ColumnReducerDouble(new DoubleArrayColumn(data), 1000, (d, e) -> e),
				(data, data2) -> {
		...
		tests.add(new CalculatorTest<>("reduce_more",
				(data, data2) -> new ColumnsReducer<>(new Column[]{new DoubleArrayColumn(data), new DoubleArrayColumn(data2)},
						() -> new double[2],
						(t, row) -> {
		...
		tests.add(new CalculatorTest<>("filter_one",
				(data, data2) -> new ColumnFilterer(new DoubleArrayColumn(data), d -> d > 0.5),
		...
		tests.add(new CalculatorTest<>("filter_more",
				(data, data2) -> new ColumnsFilterer(new Column[]{new DoubleArrayColumn(data), new DoubleArrayColumn(data2)}, row -> row.get(0) * row.get(1) > 0.3),
	...
	public static class CalculatorTest<T> {
		final String name;
		final BiFunction<double[], double[], ParallelExecutor.Calculator<T>> calculatorSupplier;
		final BiFunction<double[], double[], double[]> resultCalculator;
		final Function<ParallelExecutor.Calculator<T>, double[]> resultInterpreter;


		private CalculatorTest(String name, BiFunction<double[], double[], ParallelExecutor.Calculator<T>> calculatorSupplier,
							   BiFunction<double[], double[], double[]> resultCalculator,
							   Function<ParallelExecutor.Calculator<T>, double[]> resultInterpreter) {
			this.name = name;
	...
	public void testNumberOfOperations() {
		...
		BiFunction<double[], double[], ParallelExecutor.Calculator<?>> supplier = test.calculatorSupplier;
		ParallelExecutor.Calculator<?> calculator = supplier.apply(data, data);
	...
	public void testFullPart() {
		...
		BiFunction<double[], double[], ParallelExecutor.Calculator<?>> supplier = test.calculatorSupplier;
		ParallelExecutor.Calculator<?> calculator = supplier.apply(data, data2);
		calculator.init(1);
		calculator.doPart(0, size, 0);

		Function<ParallelExecutor.Calculator<?>, double[]> resultInterpreter = test.resultInterpreter;
	...
	@Test
	public void testReducingSeveralPartsInParallel() throws InterruptedException {
		int batches = 15;
		int factor = 5;
		int size = batches * factor;
		double[] data = random(size);
