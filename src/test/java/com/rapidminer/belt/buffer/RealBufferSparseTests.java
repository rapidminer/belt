...
/**
 * Tests for {@link RealBufferSparse}.
 *
 * @author Kevin Majchrzak
 */
public class RealBufferSparseTests {
	...
	private static double[] random(int n) {
		...
		Arrays.setAll(data, i -> Math.random() < SPARSITY ? defaultValue : Math.random());
		return data;
	}
	...
	public void testSet() {
		...
		RealBufferSparse buffer = Buffers.sparseRealBuffer(ColumnTestUtils.getMostFrequentValue(testData, -17.3), n);
		for (int i = 0; i < n; i++) {
	...
	public void testNumberOfNonDefaults() {
		...
		RealBufferSparse buffer = Buffers.sparseRealBuffer(defaultValue, n);
	...
	public void testGrow() {
		int n = 1_000_000;
		double[] testData = random(n);
		RealBufferSparse buffer = new RealBufferSparse(ColumnTestUtils.getMostFrequentValue(testData, -17.3), n);
		for (int i = 0; i < n; i++) {
			buffer.setNext(testData[i]);
		}
		Column column = buffer.toColumn();
	...
	public void testBufferLength() {
		RealBufferSparse buffer = new RealBufferSparse(Math.random(), 197);
		assertEquals(197, buffer.size());
	}

	@Test(expected = IllegalArgumentException.class)
	public void testBufferNegativeLength() {
		Buffers.sparseRealBuffer(Math.random(), -5);
	}

	@Test
	public void testZeroBufferLength() {
		RealBufferSparse buffer = new RealBufferSparse(Math.random(), 0);
	...
	public void testNotFinite() {
		...
		RealBufferSparse buffer = new RealBufferSparse(ColumnTestUtils.getMostFrequentValue(data, -17.3), data.length);
	...
	public void testIllegalIndex() {
		RealBufferSparse buffer = new RealBufferSparse(10, 100);
	...
	public void outOfLowerBounds() {
		RealBufferSparse buffer = Buffers.sparseRealBuffer(random(1)[0], 100);
