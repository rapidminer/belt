...
public class MixedRowWriterTests {
	...
	private static final ColumnType<String> TEXT = ColumnTypes.freeType("text", String.class, null);
	...
	public static class InputValidation {
		...
		public void testNullLabels() {
			Writers.mixedRowWriter(null, Arrays.asList(ColumnTypes.INTEGER, ColumnTypes.NOMINAL));
		}

		@Test(expected = NullPointerException.class)
		public void testNullLabelsPosRows() {
			Writers.mixedRowWriter(null, Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.NOMINAL), 1);
		}

		@Test(expected = IllegalArgumentException.class)
		public void testEmptyLabels() {
			Writers.mixedRowWriter(new ArrayList<>(), Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.REAL));
		}


		@Test(expected = IllegalArgumentException.class)
		public void testEmptyLabelsPosRows() {
			Writers.mixedRowWriter(new ArrayList<>(), Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.TIME), 1);
		}
		...
		public void testMismatchedTypesLength() {
			Writers.mixedRowWriter(Arrays.asList("a", "b"), Collections.singletonList(ColumnTypes.NOMINAL));
		}

		@Test(expected = IllegalArgumentException.class)
		public void testMismatchedTypesLengthWithRows() {
			Writers.mixedRowWriter(Arrays.asList("a", "b"), Collections.singletonList(ColumnTypes.NOMINAL), 5);
		}

		@Test(expected = IllegalArgumentException.class)
		public void testInvalidLabels() {
			Writers.mixedRowWriter(Arrays.asList("a", "a"), Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.INTEGER), 5)
					.create();
		}
	...
	public static class RowEstimation {
		...
		public void testTooSmallRows() {
			...
			MixedRowWriter writer = Writers.mixedRowWriter(Arrays.asList("a", "b", "c", "d", "e", "f"),
					Arrays.asList(ColumnTypes.REAL, ColumnTypes.NOMINAL, ColumnTypes.TIME, TEXT, ColumnTypes.DATETIME,
							ColumnTypes.INTEGER), numberOfRows / 2);
			for (int i = 0; i < numberOfRows; i++) {
				writer.move();
		...
		public void testExactRows() {
			...
			MixedRowWriter writer = Writers.mixedRowWriter(Arrays.asList("a", "b", "c", "d", "e", "f"),
					Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.DATETIME, ColumnTypes.INTEGER, ColumnTypes.REAL,
							TEXT, ColumnTypes.TIME), numberOfRows);
			for (int i = 0; i < numberOfRows; i++) {
				writer.move();
		...
		public void testCreateTwiceDatetime() {
			MixedRowWriter writer = Writers.mixedRowWriter(Arrays.asList("b", "c", "d"),
					Arrays.asList(ColumnTypes.INTEGER, TEXT, ColumnTypes.TIME), 2);
			writer.create();
			writer.create();
		}
		...
		public void testCreateTwiceTime() {
			MixedRowWriter writer = Writers.mixedRowWriter(Collections.singletonList("d"),
					Collections.singletonList(ColumnTypes.REAL), 2);
			writer.create();
			writer.create();
		}
	}
		...
		public void testKnownRows() {
			...
			MixedRowWriter writer = Writers.mixedRowWriter(Arrays.asList("a", "b", "c", "d", "e", "f"),
					Arrays.asList(ColumnTypes.NOMINAL, ColumnTypes.DATETIME, ColumnTypes.REAL, ColumnTypes.TIME,
							ColumnTypes.INTEGER, TEXT), numberOfRows / 2);
			for (int i = 0; i < numberOfRows; i++) {
				writer.move();
				writer.set(0, "" + first[i]);
				writer.set(3, null);
