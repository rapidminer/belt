...
/**
 * Tests {@link ColumnReducer}, {@link ColumnReducerDouble} and {@link ColumnsReducer}.
 *
 * @author Gisa Meier
 */
...
public class ColumnReducerTests {
	...
	public static class Parts {
		...
		public void testOneColumn() {
			...
			ColumnReducer<double[]> calculator = new ColumnReducer<>(new DoubleArrayColumn(data), () -> new double[2],
					(t, d) -> {
						t[0] += d;
						t[1] += 1;
					},
		...
		public void testOneColumnDouble() {
			int size = 75;
			double[] data = random(size);
			ColumnReducerDouble calculator = new ColumnReducerDouble(new DoubleArrayColumn(data), 0,
					(d, e) -> d + e + d * e);
			calculator.init(1);
			int start = 10;
			int end = 30;
		...
		public void testTwoColumns() {
			...
			ColumnsReducer<double[]> calculator = new ColumnsReducer<>(new Column[]{new DoubleArrayColumn(data),
					new DoubleArrayColumn(data2)}, () -> new double[2],
					(t, row) -> {
		...
		public void testThreeColumns() {
			...
			ColumnsReducer<double[]> calculator = new ColumnsReducer<>(new Column[]{new DoubleArrayColumn(data),
					new DoubleArrayColumn(data2),new DoubleArrayColumn(data3)}, () -> new double[2],
					(t, row) -> {
